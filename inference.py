...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key, value in self.items():
            ...
            probs.append(float(value) / self.total())
        x = random.random()
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        positionList = self.allPositions
        pacmanPos = gameState.getPacmanPosition()
        jailPos = self.getJailPosition()

        for position in positionList:
            observeProb = self.getObservationProb(observation, pacmanPos, position, jailPos)
            beliefUpdate[position] = observeProb * self.beliefs[position]

        beliefUpdate.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        beliefPred = DiscreteDistribution()
        positionList = self.allPositions

        for oldPos in positionList:
            newPosDist = self.getPositionDistribution(gameState, oldPos)

            for p in newPosDist:
                beliefPred[p] += self.beliefs[oldPos] * newPosDist[p]

        self.beliefs = beliefPred
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for location in self.particles:
            ...
            for i in range(ghostNum):
                ...
                observeProb *= self.getObservationProb(observation[i], pacmanPos, location[i], jailPos)
        ...
        if beliefUpdate.total() == 0:
            ...
            self.beliefs = beliefUpdate
            self.beliefs.normalize()
            self.particles = [self.beliefs.sample() for i in range(particleNum)]
